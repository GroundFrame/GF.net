...
namespace GF.Rails
{
    public class Railway
    {
        ...
        private readonly List<IBusinessUnit> _logicalStructure;
        private readonly List<Place> _places;
        private readonly List<ISignalBox> _signalBoxes;
        ...
        /// </summary>
        [JsonIgnore]
        public IEnumerable<IBusinessUnit> LogicalStructure { get { return this._logicalStructure; } }
        ...
        /// <summary>
        /// Gets the collection of signal boxes
        /// </summary>
        ...
        public Railway()
        {
            this._logicalStructure = new List<IBusinessUnit>();
            this._places = new List<Place>();
            this._signalBoxes = new List<ISignalBox>();
            ...
            {
                return new Length(((fromNode as ISignalBlock).Length.Meters / 2) + ((toNode as ISignalBlock).Length.Meters / 2));
        ...
        /// <param name="startSignalBlock">An optional start <see cref="ISignalBox"/> if the path needs to start at a specific signal block. Must be associated to the <paramref name="start"/> timing block</param>
        /// <param name="endSignalBlock">An optional end <see cref="ISignalBox"/> if the path needs to start at a specific signal block. Must be associated to the <paramref name="start"/> timing block</param>
        ...
        public TimingPoint AddTimingPoint(string name, string key, double latitude, double longitude, DateTime startDate, TimingPointType type, IBusinessUnit region, Place place = null)
        {
        ...
        public void AddSignalBox(ISignalBox signalBox, bool addBlocksToSignalBlockGraph = false)
        {
            if (this._signalBoxes.Any(sb => sb.Id.Equals(signalBox.Id)))
            {
                int index = this._logicalStructure.FindIndex(sb => sb.Id.Equals(signalBox.Id));
                this._signalBoxes[index] = signalBox;
                return;
            }

            this._signalBoxes.Add(signalBox); 
            
            if (addBlocksToSignalBlockGraph)
            {
                foreach (var signalBlock in signalBox.PanelOrWorkstations.SelectMany(pw => pw.GetPanelItems<SignalBlock>((DateTime)signalBox.GetStartDate())))
                {
        ...
        public void AddBusinessUnitToLogicalStucture(IBusinessUnit businessUnit)
        {
            if (this._logicalStructure.Any(r => r.Id.Equals(businessUnit.Id)))
            {
                int index = this._logicalStructure.FindIndex(r => r.Id.Equals(businessUnit.Id));
                this._logicalStructure[index] = businessUnit;
                return;
            }

            this._logicalStructure.Add(businessUnit);
        }

        public Place AddPlace(string name, string key, DateTime startDate, PlaceType type, IBusinessUnit region)
        {
        ...
        public void AddPlace(Place place)
        {
            ...
            {
                int index = this._logicalStructure.FindIndex(p => p.Id.Equals(place.Id));
