...
namespace GF.SimSig.Gateway.Stomp.IO
{
    ...
    public sealed class Stomp12FrameWriter : IStompFrameWriter
    {
        ...
        {
            //Make sure IO operations do not get executed on UI thread.
            await WriteFrameAsyncImpl(data, cancellationToken).ConfigureAwait(false);
        }
        ...
        private async Task WriteFrameAsyncImpl(byte[] data, CancellationToken cancellationToken)
        {
#if TRACE
